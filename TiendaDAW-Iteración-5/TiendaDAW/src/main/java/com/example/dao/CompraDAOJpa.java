...
public class CompraDAOJpa implements CompraDAO,Serializable{
    ...
    public List<Compra> buscaTodosUsuario(String emailbuscado) {
        List<Compra> lista = null;
        try {
            Query q = em.createQuery("Select c from Compra c where c.email = :email", Compra.class);
            q.setParameter("email",emailbuscado);
            lista = (List<Compra>)q.getResultList();
        } catch (Exception ex) {
            logger.log(Level.SEVERE, ex.getMessage(), ex);
        }
    ...
    public boolean guarda(Compra c) {
        ...
        int id=this.getUltimoIDCompra()+1;
        c.setId(id);
        try {
            em.persist(c);
            guardado = true;
    ...
    public boolean borra(Compra ce) {
        ...
        try {
            Compra c = null;
            c = em.find(Compra.class, ce.getId());
            em.remove(c);
            borrado = true;
        } catch (Exception ex) {
            logger.log(Level.SEVERE, ex.getMessage(), ex);
